...
puzzleFunctions = [p2]
def generate(puzzleMatrix, puzzleNumber, slice_index):
    count = np.zeros(65)
    colorIndex = 1
    layerNum = 0
    limit = 65*3
...
def organizePuzzleDFS(puzzleMatrix, puzzleNumber):
    stack = []
    visited = set()

    def dfs(current_puzzle):
        if tuple(current_puzzle.flatten()) in visited:
            return

        visited.add(tuple(current_puzzle.flatten()))

        if checkStack(current_puzzle):
            stack.append(np.copy(current_puzzle))
            return

        for i in range(1, 66):  # Try adding slices in a specific order (modify as needed)
            slice_index = i - 1
            if puzzleFunctions[puzzleNumber](slice_index + 1) <= 65:  # Filter out invalid indices
                next_puzzle = np.copy(current_puzzle)
                generate(next_puzzle, puzzleNumber, slice_index)
                dfs(next_puzzle)

    initial_puzzle = np.zeros([65, 3], dtype=int)
    dfs(initial_puzzle)

    return stack

 #ITERATES THROUGH PUZZLES 1-4
puzzle = np.zeros([65, 3], dtype=int) #intialize Empty puzzle matrix

puzzle = min(organizePuzzleDFS(puzzle, 0), key=lambda x: len(x))
